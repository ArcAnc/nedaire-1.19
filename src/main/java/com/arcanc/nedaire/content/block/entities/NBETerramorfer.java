...
public class NBETerramorfer extends NBERedstoneSensitive implements IInventoryCallback, ModServerTickerBlockEntity 
{
	...
	public NBETerramorfer(BlockPos pos, BlockState state) 
	{
		...
		poses = new BlockPos[] {pos.north(2),
								pos.south(2),
								pos.west(2),
								pos.east(2)};
	}
			...
			for (int q = 0; q < 4; q++)
			{
				...
				if (state.getBlock() == Blocks.STONE)
				{
					...
					else 
					{
						...
						{
							getLevel().setBlockAndUpdate(poses[q], NRegistration.RegisterBlocks.SKYSTONE.getDefaultBlockState());
	...
	public void onDataPacket(Connection net, ClientboundBlockEntityDataPacket pkt) 
	{
		...
		if (this.hasLevel() && this.getLevel().isClientSide())
		{
			for (int q = 0; q < 4; q++)
			{
				if(placedTime[q] != -1)
				{
					EssenseRender.addNewPoint(new Vec3(getBlockPos().getX() + 0.5d, getBlockPos().getY() + 0.5d, getBlockPos().getZ() + 0.5d), 
											  new Vec3(poses[q].getX() + 0.5d, poses[q].getY() + 0.5d, poses[q].getZ() + 0.5d));
				}
				else
				{
					EssenseRender.removePoint(new Vec3(getBlockPos().getX() + 0.5d, getBlockPos().getY() + 0.5d, getBlockPos().getZ() + 0.5d), 
											  new Vec3(poses[q].getX() + 0.5d, poses[q].getY() + 0.5d, poses[q].getZ() + 0.5d));
				}
			}
